...
mod tests {
    ...
    fn misencoded_bit_string_flags() {
        ...
        let mut empty_bit_string = untrusted::Reader::new(untrusted::Input::from(&[
            BITSTRING_TAG, // BitString
            0x1,           // 1 byte of content.
            0x00,          // No padding and no bit flags.
        ]));
        assert!(matches!(
            bit_string_flags(&mut empty_bit_string),
            Ok(flags) if !flags.bit_set(0)
        ));
